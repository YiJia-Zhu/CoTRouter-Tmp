# plot_results.py
"""
Plotting script for CoTRouter experimental results
"""
import json
import os
import numpy as np
...
def plot_pareto_frontier(analysis, dataset_name, save_path):
    ...
    for method, stats in methods.items():
        ...
        if 'CoTRouter' in method:
            cotrouter_methods.append(point)
    ...
    if cotrouter_methods:
        x = [m['llm_ratio'] * 100 for m in cotrouter_methods]
        y = [m['accuracy'] * 100 for m in cotrouter_methods]
        plt.scatter(x, y, s=200, marker='o', label='CoTRouter', zorder=5)
        
        # Connect CoTRouter points
        sorted_methods = sorted(cotrouter_methods, key=lambda m: m['llm_ratio'])
...
def plot_ablation_study(analysis, dataset_name, save_path):
    ...
    ablation_methods = ['CoTRouter-Full', 'CoTRouter-NoKalman', 
                       'CoTRouter-NoAdaptive', 'CoTRouter-NoCommitment']
    ...
    for method in ablation_methods:
        ...
        methods.append(method.replace('CoTRouter-', ''))
...
def plot_entropy_distribution(results, dataset_name, save_path):
    ...
    # Collect all entropy values from CoTRouter runs
    all_entropies = []
    llm_entropies = []
    slm_entropies = []
    
    for method, data in results[dataset_name].items():
        if 'CoTRouter' in method and data['metrics']:
    ...
    plt.subplot(2, 1, 2)
    if results[dataset_name] and 'CoTRouter-R30' in results[dataset_name]:
        first_metric = results[dataset_name]['CoTRouter-R30']['metrics'][0]
...
def plot_comparison_table(analysis, dataset_name, save_path):
    ...
    key_methods = ['SLM-Only', 'LLM-Only', 'Random-P50', 'FixedThreshold-T3.0',
                   'CoTRouter-R20', 'CoTRouter-R30', 'CoTRouter-R40']
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Plot CoTRouter results")

...
"""
python www1.py --results_file /mnt/8T/xgr/shizhenning/y4_big_samll/results/param_sweep/GSM8K_20250730_205653_7/results.json --method_name Sweep_1
python www1.py --results_file ./results/param_sweep/MATH_20250730_220604_7/results.json --method_name Sweep_1
python www1.py --results_file ./results/param_sweep/AIME_20250730_175240_7/results.json --method_name Sweep_1
"""
import json
import numpy as np
import argparse
...
def analyze_switch_points(results_file: str, method_name: str):
    ...
    # 新增统计结构
    dataset_switch_stats = {} # 存储每个数据集的统计信息
    ...
    for dataset, methods in results_data.items():
        ...
        switches_per_sample = [] # 存储每个样本的切换次数
        ...
        for i, (pred_item, metric_item) in enumerate(zip(predictions, metrics)):
            ...
            # 注意：这里我们使用 len(entropy_history) 作为生成的 token 数更精确
            # 因为 entropy_history 是与生成的 token 一一对应的。
        ...
        dataset_switch_stats[dataset] = {
            ...
            'switches_per_sample_distribution': switches_per_sample # 用于直方图绘制
        }
    ...
    print("="*80)
    print(f"{'Dataset':<15} | {'总切换次数':<10} | {'样本数':<10} | {'平均切换次数':<12} | {'每百 Token 切换次数':<18}")
    print("-" * 78)
    for dataset, stats in dataset_switch_stats.items():
        print(f"{dataset:<15} | {stats['total_switches']:<10} | {stats['total_samples']:<10} | {stats['avg_switches_per_sample']:.4f}{'':<8} | {stats['switches_per_100_tokens']:.4f}{'':<14}")
